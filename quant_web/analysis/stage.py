...
# 主力阶段只是按量价规则给走势起的名字，不是买卖信号。历史统计（2022 年以来每周，量化选股的可买股票池，
# 下一周相对同池平均；multifactor.service.stage_check）："拉升" −0.43%/周（t −4.2，最差）、"出货" −0.18%（t −2.9），
# "下跌" +0.13%、"吸筹" +0.05%、"洗盘" −0.04%、"不明确" +0.03%（都和平均差不多）。advice 只写这些事实，不给没验证过的买卖规则。
STAGES: dict[str, dict] = {
    "accumulation": {"label": "吸筹", "tone": "neutral",
                     "desc": "主力可能在低位悄悄买入：股价低位横盘、波动变小、上涨放量下跌缩量。",
                     "advice": "这是按量价规则起的名字，不是买入信号：历史上被判为“吸筹”的股票，之后一周和同期平均差不多。"},
    "washout": {"label": "洗盘", "tone": "neutral",
                "desc": "涨了一段后缩量回调、关键均线不破，像是在把不坚定的人吓出去。",
                "advice": "这是按量价规则起的名字，不是买卖信号：历史上被判为“洗盘”的股票，之后一周和同期平均差不多。"},
    "markup": {"label": "拉升", "tone": "watch",
               "desc": "放量突破、均线多头、量价齐升，趋势向上。",
               "advice": "看起来很强，但历史上被判为“拉升”的股票，之后一周平均跑输同期其他股票（统计上显著）：不要因为“拉升”去追、去加仓。"},
    "distribution": {"label": "出货", "tone": "bad",
                     "desc": "高位放量滞涨、量价背离或放量破位，像是大资金在卖给后来者。",
                     "advice": "历史上被判为“出货”的股票，之后一周平均跑输同期其他股票：不建议这时追进去；已经持有的按你自己的计划（止损 / 调仓）处理。"},
    "decline": {"label": "下跌", "tone": "neutral",
                "desc": "均线空头、低点不断下移、反弹无量，处在下跌趋势中。",
                "advice": "这只说明最近在跌：历史上被判为“下跌”的股票，之后一周并不比平均差；“等站上 60 日线再买”这类规则没有经过验证，不用机械执行。"},
    "unclear": {"label": "不明确", "tone": "neutral",
                "desc": "各阶段的证据都不充分，看不出明显的主力行为。",
                "advice": "这个标签本身不说明会涨还是会跌。"},
}
