...
- 因子回看期：`dataset.FEATURE_LOOKBACK_DAYS = 600` 个日历日（覆盖最长 250 个交易日的窗口，递推类指标也有足够预热）。算因子时，公式表只取"最早需要的日期 − 600 天"之后的数据，训练和给最新一天打分用同一个回看期，保证两边口径一致。
- 实验室读数据（2026-09-25 真实数据实测后改）：`dataset.load_base(boards=, codes=)`。
  - 做法：缓存还没有时先调用一次 `load_or_build_history(use_chips=False)` 生成；之后用 `scan_parquet` 只读范围内股票的 `TABLE_COLS` + 主力列，公式表只读这些股票（`fengine.load_frame(codes=)`）。
  - 实测（主板、2020 年起、4 组因子、370 万行 × 103 个因子）：
    - 读数据：全市场全部读入 6.1GB，只读需要的部分 3.9GB；
    - 建数据峰值：10.1GB → 8.6GB；
    - 第一次整理全历史字段表峰值约 14GB（890 万行）。
  - 内存估算：max(第一次整理的峰值 1.6GB / 百万行全市场行, 0.45GB / 百万行范围内行 + 0.3 + 因子表 × 3.5 + Alpha 分块)。
  - 真实数据检查（主板、2025 年起、6 组共 343 个因子，只建数据、不训练）：
    - 111 万行，97% 有标签，同日平均超额为 0（误差 4e-9）；
    - 各组覆盖率 97%~100%，没有无穷值；
    - 约 6.6 分钟（估计 8 分钟）。

## 9. 量化选股（多因子 + LightGBM，2026-09-25；核心功能）

### 9.0 研究结论（真实数据，全部可复现；不要在没有新证据时推翻）
- 数据：主板（60/00）日线 2019-01 ~ 2026-09（含已退市股票，收益用交易所昨收算，除权正确）；财报按公告日（notice_date）之后的下一个交易日才用；
  **ST 用 baostock 逐日 isST 的历史状态**（`stock_lab/bs_hist/`，3385/3389 只；2019 年以来 587 只当过 ST，其中 372 只现在已摘帽——
  旧的"当前名称"近似会让回测偏乐观）；估值（peTTM/pbMRQ）历史也来自 baostock，之后每天收盘用腾讯快照（字段 39 = 市盈率TTM、46 = 市净率，已逐只核对与 baostock 一致）补到 `stock_lab/valuation_tx.parquet`。
- 36 个候选因子（`multifactor/factors.py`）全部来自公开研究，**方向事先按文献定死**，不看回测结果调方向（反转、理想反转、换手、异常换手、换手波动、波动率、特质波动、MAX、偏度、
  振幅、理想振幅、Beta、近期涨停次数、隔夜/日内收益、跳空-换手相关、价量相关、非流动性、动量（对照）、52 周高点、市值、股价、EP/BP/SP/CFP、ROE、毛利率、应计、负债率、SUE、利润/营收增速）。
  单因子周度 RankIC（2020-2024 / 2025 至今）：特质波动 0.086/0.074、理想振幅 0.059/0.064、近期涨停次数 0.063/0.055 等显著；SUE、增速、ROE 几乎为 0；隔夜收益方向与文献相反（不用）。
- **线性打分不行、LightGBM 行**（周调仓、前 50 名、排名掉出 150 才卖、单行业 ≤ 8 只、500 万资金、按资金规模算冲击成本，样本外 2022-01 ~ 2026-09）：
  文献等权 +2.2%/年（t 0.4）、滚动 ICIR 加权 +0.1%、三者平均 +5.4%（t 1.2）、**LightGBM +13.9%/年（t 2.33），年化 22.1% vs 同池随机 6.9%，跑赢 60/60 个同换手同费用的随机组合，最大回撤 −21.5% vs −26.3%**。
  线性打分的 IC 并不差（0.084~0.099），但因子主要作用在"避开差股票"一侧，前 50 名扣完换手成本就没了。
- 稳健性：3 个随机种子单独都为正（+8.3%/+10.1%/+12.8%）；24 种组合构建（前 30/50/100、保留 2/3 倍、是否平滑、是否行业上限）全部为正（+8%~+13.5%）；
  2022-2024 与 2025 至今两段都为正（+10.0%/+20.4%）；把训练标签在截面内打乱的安慰剂检验 IC ≈ 0；把单笔收益最高的 1% 截掉后 2025 年仍 +95%（不是个别股票撑出来的）。
- 逐年（500 万）：2022 +16.6% vs −2.2%；2023 +11.6% vs −0.6%；2024 −0.4% vs +0.7%（小盘股 2024 年初崩盘，基本持平）；2025 +95.9% vs +47.8%；2026 至 9 月 −1.0% vs −6.0%。
- **容量**（单只下单金额 ≤ 该股 20 日日均成交额 5%，冲击成本 = 0.05% + 0.8 × 20 日波动率 × sqrt(下单额/日均成交额)）：
  100 万 +15.4%（t 2.6）、500 万 +13.9%（2.3）、2000 万 +11.1%（1.9）、5000 万 +7.9%（1.3）、1 亿 +1.4%（0.2）、3 亿 −2.6%。**上亿资金基本没有优势**，页面必须显示容量表。
- 组合画像：市值分位 0.37（偏中小盘）、换手分位 0.35、波动分位 0.26；模型最看重"近 60 日涨停次数"（重要性约 39%，避开炒作过的股票）、5 日涨幅、Beta、市值、特质波动。
- 以前的结论"选股方法都没有显著跑赢随机"只适用于选股器的条件方案、经典公式、主力阶段标签（这些仍然没有跑赢），首页/指南/策略中心/明日计划的文案已改。
- 选择偏差：LightGBM 是在 4 种方法里挑出来的（其余都公开展示在"为什么用 LightGBM"里）；页面提示实盘大概率比回测差，以前向跟踪为准。

### 9.1 代码 `quant_web/multifactor/`
- `panel.py`：主板日线 → 宽表（`Panel`：复权开/收、可交易、开盘涨停/跌停、收盘涨停、上市天数、历史 ST、估值）；`from_wide()` 供测试和每日打分；
  `fund_asof()` 财报按"公告后下一个交易日"生效、同日多期取最新、旧报告期补发不回退、新一期缺值不沿用旧值；`single_quarter()` 累计值 → 单季。
- `factors.py`：因子注册表 `FACTORS`（key、中文名、类别、先验方向、说明、计算函数）+ `Ctx` 公共中间量（对数收益、等权市场收益、流通市值 = 成交量/换手率 × 收盘价的 20 日中位数、财报宽表）。
- `evaluate.py`：股票池 `eligible()`、调仓日 `rebalance_dates()`（每周/每月最后一个交易日）、`forward_open_returns()`（t+1 开盘买 → 下一调仓日 +1 开盘卖；买入日开盘涨停 = 买不进）、
  `rank_ic`、`quantile_returns`、`zscore`（截面排名 → 均匀分布 z 分）、`composite`、`backtest()`（逐期模拟：开盘涨停买不进顺延下一名、停牌或开盘跌停卖不出继续拿、只对买卖部分收费、
  行业上限、同池等权基准（不扣费，对基准更有利）、换手率匹配的蒙特卡洛随机组合）、`impact_slippage()` / `participation()`（按资金规模的冲击成本与流动性）。
- `model.py`：`Spec`（method/freq/keys/训练期数/重训间隔/lag=2/seeds），`scores()` 给出 LIT / ICIR / LGB / ENS；**第 t 期只用标签完整已知的第 t−2 期及以前训练**。
  `PROD_KEYS` = 去掉 sp/cfp 的 34 个因子（这两个只有 baostock 历史，最新几天拿不到）。
- `valuation.py`：腾讯快照补当天估值。`service.py`：生产配置（`SPEC` = LGB 周频 3 种子、`TOP_N`=50、`KEEP_RANK`=150、`INDUSTRY_CAP`=8、`MIN_AMOUNT`=2000 万、`MAX_PARTICIPATION`=5%）、
  `universe_mask()`（主板、非 ST、上市满一年、20 日日均成交额 ≥ 2000 万、股价 ≥ 2 元、不是"亏损且营收 < 3 亿"）、`capacity_mask()`、`run_daily()`（→ `workspace/multifactor/today.json`；
  本周最后一个交易日还会 `record_track()` 记进 `track.json`，记下后不改）、`track_performance()`（前向跟踪真实成绩）、`build_report()`（→ `report.json`，约 5~6 分钟）。
- 后台任务：`mf_daily`、`mf_report`（tasks.py，HEAVY 组）；投资助手每日流水线在"选股方案"之后调用 `run_daily()`。
- 接口 `api/routes/multifactor.py`：`GET /api/mf/today`、`GET /api/mf/report`、`POST /api/mf/run`、`POST /api/mf/rebuild`、`GET /api/mf/track`、`GET /api/mf/plan?capital=`（等权、100 股一手向下取整、和上一期比列出买/卖/继续持有）。
- 页面 `#/mf`（导航"选股"组第一项、手机底栏）：结论横幅（样本外、扣费、同池随机、t 值、随机组合百分位、回撤、比中证1000、风险与选择偏差提示）→ 本周组合 / 全部排名 / 要卖出 + 下单清单 →
  样本外净值（含指数、随机组合 10%~90% 区间）→ 容量表 → 逐年 → 随机对照直方图 → 方法对照 → 因子表 → 组合画像 → 前向跟踪。
- 研究脚本（未入库，在会话 scratchpad）：research1（单因子）、research2（方法 × 频率 × 持股数）、research3（种子与组合构建网格）、research4（容量、画像、重要性）、research5（安慰剂）。
- 测试 `tests/test_quant_web_multifactor.py`：除权收益、涨跌停与 ST 5%、财报时点（公告当天不可用、同日取最新、补发不回退、缺值不沿用）、单季拆分、调仓日、开盘涨停买不进、跌停卖不出、费用、z 分、因子方向、下单清单、目标组合规则。

## 10. 连板页 `#/lianban`（2026-09-25 恢复为一级入口）
- 背景：第三版把"短线预测"从顶部标签挪进了折叠分组并改名"短线观察"，用户以为连板功能没了。现在"连板"是行情组的一级入口，"短线预测"恢复原名。
- `market/lianban.py`（复用 `predict.service.context()` 的 `panel_lim` 与情绪表）：`ladder(day, main_only)` 某天涨停股按连板高度分层——优先用涨停池存档（封板资金、首次/最后封板时间、炸板次数、几天几板），
  没存档的日子用日线识别；过去的日子附带次日开盘/收盘涨幅与是否晋级；最新一天附带连板晋级模型概率（观察用）。`promotion(days, main_only)` 各高度次日晋级率、开盘溢价、收盘涨幅、红盘率、一字率；`cycle(days)` 情绪序列。
- 实测（主板、近一年）：首板→2 板 17.5%、2→3 板 32.9%、3→4 板 46%、4→5 板 44%；炸板股次日平均 −0.6%。一字板买不进，实际能拿到的比平均值差。
- 接口 `api/routes/lianban.py`：`GET /api/lianban/ladder?date=&main_only=`、`GET /api/lianban/stats?days=&main_only=`（晋级率 + 一年情绪周期 + 可选日期）。
- 页面：日期选择（前后一天）、只看主板、8 个情绪指标、天梯（每层"几只、晋级几只"，次日晋级的绿框）、晋级率表、涨停行业分布（点击高亮天梯）、情绪周期双图（点图切日期）、涨停/炸板/跌停池表（独立滚动框）。

## 11. 布局规范（2026-09-25 重构）
- `.qw-content` 最大宽度 2400px（原 1600px）；先横向铺满再向下；同类长列表放进独立滚动框（max-height ≈ 视口高度减页头，或 480~640px）。
- 公式库：列表卡片内滚动（`.fm-list > .qw-card-bd` 与 `.fm-items` 的 flex 链）、分组标签换行；右侧两栏（公式详情 | 在个股上看信号），历史验证横跨两栏、三个持有期并排。
- 看盘：上方 K 线（约 2/3）与主力阶段（约 1/3，独立滚动）并排，其余卡片 `repeat(auto-fill, minmax(360px, 1fr))` 网格，相关新闻两栏。
...
## 13. 实盘前全面审计（2026-09-25）：修正与依据
用户要投入大资金，要求"不能误导、不能出错"。六个方面分头审计（量化选股流水线、交易与风控、误导性文字、分析与回测计算、数据与每日流水线、最近的代码），
下面是确认过的问题和改法；涉及"要不要这样做"的地方都先用真实数据验证（脚本在会话 scratchpad：regime_mf / loss_mf / stop_mf / stage_vs_mf）。
- **用数据回答的三个问题**（量化选股，样本外 2022-01 ~ 2026-09，500 万口径）：
  - 按大盘环境减仓：用户设置 0.7/0.7/0.4 → 年化 22.1% → 12.2%、夏普 0.90 → 0.85；文档默认 0.8/0.5/0.2 → 9.9%、0.77；弱势空仓 → 9.6%、0.59。
    弱势周组合收益反而最高（+0.59%/周，超额 +0.35%）。结论：**量化选股不按大盘环境减仓**；想少担风险就固定少投。仓位上限的文字全部改成"你设置的经验规则，不是涨跌预测"。
  - 排雷的"亏损"红灯：持仓里近四季亏损股（20%）下一周超额 +0.70%/周，盈利股 +0.34%（同周配对差 +0.08%，t 0.4）；选股时去掉亏损股，年化 22.1% → 20.1%。
    结论：亏损、业绩预告、解禁、质押、减持是"软"红灯（只要求确认）；退市 / ST / 面值和市值退市线 / 资不抵债 / 成交极冷 / 监管处罚是"硬伤"（禁止买入）。`riskscan.HARD_KEYS`、`hard_red()`。
  - 单只止损：5% / 8% / 12% / 15% 止损每期分别比不止损 −0.029% / −0.042% / −0.018% / −0.015%（t −0.4 ~ −0.7），8% 止损年化少约 2.6 个百分点、最大回撤只小约 1 个百分点。
    结论：量化选股单只不设止损（和回测一致），页面写明数字。
  - 真实预期（`service.expectations`，存 `report.expect`）：单只持仓下一周下跌 46%、跌超 10% 1.5%；组合 43% 的周亏钱，最差一周 −16.9%、最差连续 4 周 −20.5%。页面"先有心理准备"。
- **量化选股**：
  - 当前 ST：baostock 历史 ST 只到下载那天，之后的日子叠加股票列表里的当前 ST（以前之后新戴帽的股票会被当成非 ST）。
  - 前向跟踪"已完成"：一期要等下一条记录的次日开盘卖出才算结束，最后一条永远是进行中（以前过一天就算完成）。
  - 下单清单：每只下单金额超过日均成交额 5% 标"量大"；名单是按设置里的资金筛流动性的，清单资金明显更大时提醒改资金后重新打分；新买入的涨停标记；
    候补名单（排名紧跟、守行业上限，买不进时按顺序换）；要卖出的最新一天停牌标"复牌后再卖"。
  - 下单前自检 `service.health()`（`/api/mf/today` 的 `health`，页面顶部）：日线是否到最新已收盘交易日、最新一天股票数是否比前一天少 3% 以上、名单是否按最新数据打分、
    组合里现在是否有 ST、是否停牌、组合只数、交易日历是否可用。
  - 结论横幅：按用户资金取容量表最接近的一档（资金低于最小一档时提示一手买不起、结果会差很多）；"跑赢随机组合"改成"60 / 60"。
- **每日流水线**：明日计划推送先做自检（`health()` + 前面失败的步骤），有问题时标"紧急"并写"先别按清单下单"；选股器候选只用当天的结果（不拿旧名单冒充）；
  `providers/updates.py` 等几处 `date.today()`（这台电脑是纽约时区）改成北京时间。页头"数据日期"显示"不是最新 / 可能不全"（`/api/status` 的 `freshness`）。
- **交易与实盘安全**：
  - 排队（queued）的实盘委托到了交易日收盘还没提交就过期；开盘只提交当天的，过了交易日的作废并紧急提醒；提交前按开盘价再核对单笔上限。
  - 券商接口发送时出错（超时、断线、没拿到委托编号）不再当成"被拒绝"（那样会诱导重复下单），保持"已提交"并紧急提醒去券商 App 核对（`BaseBroker.mark_uncertain`）；发出去之前的错误仍是拒绝。
  - 盘中实时撮合（`match_with_quotes`）也检查换仓买单的现金（以前只有日终撮合检查，盘中可能透支）。
  - 风控：主力阶段"拉升 / 出货"下单前确认，"下跌"不再拦；排雷软红灯确认、硬伤禁止；`min_amount_20d`（以前没有生效）现在生效（确认）；实盘单笔上限不再挡卖出。
- **误导性文字**：主力阶段的建议改成只写历史事实（"拉升"之后一周最差、"下跌"不比平均差、"站上 60 日线再买"没验证）；首页情绪不再给操作建议；
  选股器打分说明、价值成长方案补"没有跑赢随机"；明日计划"疑似出货"不再说"减仓"；阶段标签颜色中性化（只把拉升 / 出货标黄）。
- **计算口径**：公式验证 / 方案回测 / 阶段统计的 `forward_returns` 持有期间退市的按最后收盘价计（以前被当成没结束丢掉，会让爱买暴跌股的方法显得更好）；
  批量排雷的业绩预告和单只诊断同一口径；止损下限不再大于设置的止损比例；策略回测缓存键加入资金和风控设置；回测 / 验证结果超过 30 天提示重算；
  实验室"启用到选股器"对样本外没有优势的模型要求明确确认（服务器端也检查）；模拟跟踪在量化选股打分过旧时只卖不买。
- 测试：`tests/test_quant_web_safety.py`（换仓买单盘中不透支、排队委托过期、缓存键、批量排雷口径、止损下限、holdout 一致、阶段文字、下单清单、下单前自检），
  `test_quant_web_validate.py`（退市按最后收盘价），`test_quant_web_trading.py` / `test_quant_web_brokers.py` / `test_quant_web_lab.py` 按新规则更新。
